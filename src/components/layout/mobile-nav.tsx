...
export function MobileNav({ open, onClose }: MobileNavProps) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className={`fixed left-0 top-0 z-[70] flex h-full w-full max-w-[400px] flex-col bg-surface transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] ${
          open ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        {/* Header */}
        <div className="flex h-[60px] items-center justify-between px-8">
          <span className="font-logo text-sm font-bold tracking-[0.08em] text-ink uppercase">
            DUSK&CO
          </span>
          <button onClick={onClose} aria-label="Close menu" className="text-ink-muted hover:text-ink transition-colors">
        ...
        <nav className={`flex flex-col gap-1 px-8 pt-8 ${open ? "stagger-children" : ""}`}>
          {LINKS.map((link) => (
            <Link
              ...
              href={link.href}
              onClick={onClose}
              className="animate-slide-in-left block font-primary text-[13px] font-bold tracking-[0.16em] text-ink uppercase opacity-0 transition-colors hover:text-ink-muted leading-tight py-1"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="px-8 py-6">
          ...
          <a
            ...
            className="mt-3 flex items-center gap-2 font-primary text-[11px] font-light tracking-[0.06em] text-ink-faint transition-colors hover:text-ink-muted"
          >
            <Mail size={13} strokeWidth={1.5} />
            help@dusk.co
          </a>

          <p className="mt-4 font-primary text-[10px] font-light tracking-[0.1em] text-ink-faint uppercase">
            Wear the difference.
          </p>
        </div>
      </div>
